...
let printArea;   // Viewport guide – the area designs must stay inside
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Load initial product base image
    loadProductImage(toolState.currentProduct, toolState.currentColor);

    // 3. Create the visible viewport guide (your measured print area)
    createPrintArea();   // 👈 dashed box that the user can see

    // 4. Save initial state for undo/redo
    saveState();

    // 5. Hook up UI event listeners
    setupUI();

    // 6. Track changes on canvas for history
    ...
    // 7. Keep customisations inside the viewport guide on move/scale
...
// Clamp any object so it stays inside printArea
function keepObjectInPrintArea(obj) {
    if (!canvas || !printArea) return;

    // Don’t clamp the base shirt or the guide itself
    if (obj.id === 'product_base' || obj.id === 'print_area') return;

    // Bounding box including scale/rotation
    ...
    if (br.left < paLeft) {
        dx = paLeft - br.left;
    } else if (br.left + br.width > paRight) {
        dx = paRight - (br.left + br.width);
    }

    // Vertical limits
    if (br.top < paTop) {
        dy = paTop - br.top;
    } else if (br.top + br.height > paBottom) {
        dy = paBottom - (br.top + br.height);
    }
...
function loadProductImage(product, color) {
    ...
    // Save current design layers (NOT base image)
    const designObjects = canvas.getObjects().filter(
        obj => obj.id !== 'product_base'
    );
    ...
    fabric.Image.fromURL(imageUrl, function(img) {
        ...
        // Re-add previous design objects on top (includes printArea if it exists)
